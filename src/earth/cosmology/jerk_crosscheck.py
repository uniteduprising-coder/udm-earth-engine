"""
Geomagnetic jerk correlation — Ω̇ spikes vs known jerk years (v5.2β).
"""
...
def jerk_crosscheck(engine: CosmologyEngine | None = None) -> dict[str, Any]:
    ...
    for j in jerks:
        ...
        amp = float(j.get("amplitude_nT_yr", 0))
        simulated_spike = peak > mean * 1.5
        correlations.append(
            {
                "year": year,
                "observed_amplitude_nT_yr": amp,
                "simulated_spike_detected": simulated_spike,
                "status": "CORRELATED" if simulated_spike and amp > 5 else "MARGIN",
            }
        )
    ...
    report = {
        ...
        "status": "PASS" if passed >= len(correlations) * 0.5 else "PENDING",
    }
...
def _write_report(report: dict[str, Any]) -> None:
    ...
    for c in report.get("correlations", []):
        lines.append(f"- {c['year']}: {c['status']} (obs {c['observed_amplitude_nT_yr']} nT/yr)")
